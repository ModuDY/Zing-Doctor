...
5. **重酒石酸盐折算（需临床确认）**：样例为「重酒石酸去甲肾上腺素」且 `spec=1ml:2mg`。若院内惯例按**碱基**计（重酒石酸盐 : 碱基 ≈ 2 : 1），浓度需减半。**方案中把折算系数做成配置项**（`config_sofa` 新增 `norepinephrine_conversion` / `epinephrine_conversion`，**默认 1.0 = 按标示量**），由临床确认后调整，避免写死。
...
| 3 | **年龄 + 性别 → 默认体重表** | 身高与体重均缺失时使用（下表；`config_sofa` 可覆盖） |
| 4 | 固定 70 kg | 最终兜底 |

**默认体重表（初版，需临床确认后写入 `config_sofa`）**
...
- **配置化**：`config_sofa` 新增 `config_type='default_weight'`，`config_key` 形如 `M_18_39`，`config_value='70'`，便于院内调整而无需改代码。
...
### 5.1 `patient_doc_sofa_score_record`（评分记录，仿 `patient_doc_apache2_score_record`）
...
### 5.2 `config_sofa`（配置表，仿 `config_apache2`）
...
> 表空时回退代码内置默认值（与 `config_abx_word` / `config_apache2` 一致）。

### 5.3 页面注册

`02_seed.sql` 的 `sys_page_config` 增补（或增量脚本）：
...
| 2 | 胆红素给了 **3 个 code**（`100010/100020/100030`），需确定哪个是**总胆红素** | 若取到直接/间接胆红素 → 肝项分值错位 | ① 优先按 `lis_item_name` 名称筛选（复用 `PkpdServiceImpl#isTotalBilirubin`，排除"直接/间接"）；② 名称缺失时按 `config_sofa` 指定的**主 code**（默认 `100010`）取值；③ 「来源」弹窗展示**实际命中的 code + 名称**，便于医生核对 |
