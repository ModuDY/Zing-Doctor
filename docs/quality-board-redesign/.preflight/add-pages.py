...
PROJECT_DIR = Path('d:/work/ZING/quality-board-redesign')
PAGES_DIR = PROJECT_DIR / 'pages'
DESIGN_PATH = PROJECT_DIR / 'quality-board-redesign.design'

# Read existing design file
with open(DESIGN_PATH, 'r', encoding='utf-8') as f:
    design = json.load(f)

existing_ids = {node['id'] for node in design['data']}
...
new_pages = [
    {
        ...
        'title': '质控月度汇总',
        'html_file': 'quality-monthly.html',
        'html_title': '质控月度汇总 · 质控指标看板',
        'overview_title': '质控月度汇总',
        'overview_tag': '12 个月度报告',
        'overview_tag_strong': '已归档 8',
        'active_tab': 'monthly',
        'tab_label': '质控月度汇总',
        'visual_north_star': '月度汇总报告：按月份维度聚合核心质控指标，支持趋势查看与一键导出',
        'composition_pattern': 'summary-cards / monthly-table / export-actions',
        'component_plan': ['filter-bar', 'summary-cards', 'tabs', 'data-table', 'export-actions'],
    },
    {
        'id': 'page-quality-config',
        'title': '质控指标配置',
        'html_file': 'quality-config.html',
        'html_title': '质控指标配置 · 质控指标看板',
        'overview_title': '质控指标配置',
        'overview_tag': '127 条指标',
        'overview_tag_strong': '启用 118',
        'active_tab': 'config',
        'tab_label': '质控指标配置',
        'visual_north_star': '指标配置中心：启用状态、阈值、责任科室、计算周期一站式管理',
        'composition_pattern': 'config-table: metric-list / status-toggle / threshold-edit',
        'component_plan': ['filter-bar', 'summary-cards', 'tabs', 'data-table', 'toggle-switch'],
    }
]

# HTML templates for new pages
# We reuse the same CSS/head from existing pages
...
''' + get_common_scripts_and_fallback() + '''
</head>
...
def get_common_scripts_and_fallback():
...
def update_existing_page_tabs(html_path, active_tab):
    """Add the two new tabs to existing pages, preserving active state."""
    with open(html_path, 'r', encoding='utf-8') as f:
        content = f.read()

    old_tabs = '''          <button class="qb-tab" data-dom-id="tab-board">指标看板</button>
          <button class="qb-tab" data-dom-id="tab-coverage">覆盖率报告</button>
          <button class="qb-tab" data-dom-id="tab-facts">事实层</button>
          <button class="qb-tab" data-dom-id="tab-runs">计算批次</button>'''

    new_tabs = '''          <button class="qb-tab" data-dom-id="tab-board">指标看板</button>
          <button class="qb-tab" data-dom-id="tab-coverage">覆盖率报告</button>
          <button class="qb-tab" data-dom-id="tab-facts">事实层</button>
          <button class="qb-tab" data-dom-id="tab-runs">计算批次</button>
          <button class="qb-tab" data-dom-id="tab-monthly">质控月度汇总</button>
    ...
    if old_tabs not in content:
        print(f'Warning: old tabs not found in {html_path}')
        return

    content = content.replace(old_tabs, new_tabs)

    # Mark active tab
    for tab in ['board', 'coverage', 'facts', 'runs', 'monthly', 'config']:
        # deactivate all
        content = content.replace(f'class="qb-tab active" data-dom-id="tab-{tab}"', f'class="qb-tab" data-dom-id="tab-{tab}"')
    # activate current
    content = content.replace(f'class="qb-tab" data-dom-id="tab-{active_tab}"', f'class="qb-tab active" data-dom-id="tab-{active_tab}"')

    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f'Updated tabs in {html_path.name}')


def main():
    # Create new HTML pages
    (PAGES_DIR / 'quality-monthly.html').write_text(build_monthly_html(), encoding='utf-8')
    (PAGES_DIR / 'quality-config.html').write_text(build_config_html(), encoding='utf-8')
    print('Created quality-monthly.html and quality-config.html')

    # Update existing pages
    update_existing_page_tabs(PAGES_DIR / 'quality-board.html', 'board')
    update_existing_page_tabs(PAGES_DIR / 'quality-coverage.html', 'coverage')
    update_existing_page_tabs(PAGES_DIR / 'quality-facts.html', 'facts')
    update_existing_page_tabs(PAGES_DIR / 'quality-runs.html', 'runs')
    # detail page uses board tab active, but it has drawer overlay
    update_existing_page_tabs(PAGES_DIR / 'quality-detail.html', 'board')

    # Add new page nodes to design file
    base_time = 1789390203000
    new_nodes = [
        {
            "id": "page-quality-monthly",
            "title": "质控月度汇总",
            ...
            "devMetadata": {
                "htmlSrc": "pages/quality-monthly.html",
                "interactions": []
            }
        },
        {
            ...
            "devMetadata": {
                "htmlSrc": "pages/quality-config.html",
                "interactions": []
            }
        }
    ]

    design['data'].extend(new_nodes)

    # Build interactions for all pages
    all_page_ids = [node['id'] for node in design['data'] if node['type'] == 'page']
    tab_mappings = {
    ...
    for node in design['data']:
        ...
        if page_id == 'page-quality-detail':
            interactions.extend([
                {
                    "targetPageId": "page-quality-board",
                    "trigger": "click",
                    "selector": "[data-dom-id='drawer-backdrop']",
                    "hidden": True,
                    "label": "关闭抽屉"
                },
                {
                    "targetPageId": "page-quality-board",
                    "trigger": "click",
                    "selector": "[data-dom-id='drawer-close']",
                    "hidden": True,
                    "label": "关闭抽屉"
                }
            ])
        ...
        for target_id in all_page_ids:
            if target_id == page_id or target_id == 'page-quality-detail':
                continue
            tab_id = tab_mappings[target_id]
            label = f"切换到{node_title(design, target_id)}"
            interactions.append({
                "targetPageId": target_id,
                "trigger": "click",
                "selector": f"[data-dom-id='{tab_id}']",
                "hidden": True,
                "label": label
            })

        # Keep metric detail link only on board page
        if page_id == 'page-quality-board':
            interactions.append({
    ...
    print(f'Updated {DESIGN_PATH.name}')


def node_title(design, page_id):
    for node in design['data']:
        if node['id'] == page_id:
            return node['title']
    return page_id
