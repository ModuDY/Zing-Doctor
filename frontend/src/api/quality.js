...
/**
 * 配置单条指标的目标值 / 预警值（本院自管字段，源端同步不会覆盖）。
 *
 * 传 null 或空串即清空。返回该指标的组装行，页面可就地更新这一行。
 */
export function saveQualityRuleTarget(params) {
  return request.post('/quality/rules/config', null, { params, silentError: true })
}

/**
 * 计算单条指标：重算它的分子、分母原子项，再除出指标值。
 *
 * 与 recalcQuality（整批 127 条，分钟级）不同，这条秒级返回、只影响一行，
 * 返回里的 steps 说明分子/分母各自是「算了 / 跳过 / 忙 / 出错」。
 */
export function calcQualityRule(params) {
  return request.post('/quality/rules/calc', null, {
    params,
    silentError: true,
    timeout: 180000
  })
 ...
 * 对象级命中明细（按需生成）。
 *
 * includeExcluded=true 时连同「既没进分子也没进分母」的人一起返回，
 * 用于回答「这个人为什么不算进分母」—— 那正是质控争议最常见的起点，
 * 而这类人默认被明细 SQL 滤掉，常规下钻里看不到。
 */
export function fetchQualityMetricPatients(code, params) {
 ...
 * 导出对象级明细 xlsx。
 *
 * 与 exportQualityXlsx 同理用裸 axios：响应体是二进制流，走统一 request
 * 会被拦截器按 Result 包装解包而报错，因此手动注入鉴权头并声明 responseType=blob。
 */
export function exportQualityMetricPatients(code, params) {
  return axios.get('/api/quality/metric/patients/export', {
    params: { code, ...params },
 ...
 *
 * 与 {@link fetchFactFields} 分开是有原因的：后者含 patient_id / depart_code 这类
 * 与明细默认列同名的列，一旦选进患者明细，明细 SQL 就会报「列名不明确」、
 * 整份明细变空，而且页面只显示「无明细」，看不出是配置写错了。
 */
export function fetchPatientFields(fact) {
  return request.get('/quality/config/patient-fields', { params: { fact }, silentError: true })
}
...
/**
 * 指标规则配置列表（含未上板与已停用的规则）。
 *
 * 与 fetchQualityRules 的区别：那个要周期参数、带回本期值（看板用）；
 * 这个只回规则定义（配置用），并且能看到停用过的规则 —— 否则停用一次就再也找不回来。
 */
export function fetchConfigRules() {
  return request.get('/quality/config/rules')
}

/**
 * 新增或编辑一条指标规则（分子 ÷ 分母 × 放大系数）。
 *
 * ruleId 为空即新增，由服务端分配 LOCAL_n 编号 —— 不与 ICU 侧的数字 id 同空间，
 * 因此下次「同步指标规则」不会把它误判成源端已有规则而跳过。
 * 分子/分母必须是已存在的原子项（quality_xxx），服务端会校验并给出明确原因。
 */
export function saveConfigRule(rule) {
  return request.post('/quality/config/rule', rule, withWriteToken({
    silentError: true,
    timeout: 60000
  }))
}

/** 启用 / 停用规则（不删除，历史结果仍可按 ruleId 回溯） */
export function setConfigRuleStatus(ruleId, enabled = true) {
  return request.post('/quality/config/rule/status', null, withWriteToken({
