...
CREATE TABLE "zing_doctor_db_prod"."quality_count_rule" (
    ...
    "remark"              VARCHAR(1000),
    "numerator_code"      VARCHAR(64),
    "denominator_code"    VARCHAR(64),
    ...
    CONSTRAINT "pk_quality_count_rule" PRIMARY KEY ("id")
);
COMMENT ON TABLE  "zing_doctor_db_prod"."quality_count_rule" IS '质控指标规则（真正的业务指标 = 分子原子项 ÷ 分母原子项 × 放大系数；源同步自 ICU）';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."rule_id"            IS 'ICU 侧规则 id，本表唯一约束，用于幂等同步';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."numerator_code"     IS '分子原子项 code，指向 quality_metric_def.index_code';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."denominator_code"   IS '分母原子项 code，指向 quality_metric_def.index_code';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."percent_rate"       IS '放大系数 100/1000/1；值 = 分子 ÷ 分母 × 本值';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."target_value"       IS '本院自管目标值（ICU 侧未配）；达标线，留空则不判达标';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."warning_value"      IS '本院自管预警值（ICU 侧未配）；跌出此线为预警，留空则只判达标与否';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."target_direction"   IS '达标方向：UP=越高越好 / DOWN=越低越好；决定「达标」是 ≥ 还是 ≤ 目标值';
COMMENT ON COLUMN "zing_doctor_db_prod"."quality_count_rule"."status"             IS '1 启用 / 0 停用';

CREATE UNIQUE INDEX "zing_doctor_db_prod"."uk_qcr_rule" ON "zing_doctor_db_prod"."quality_count_rule" ("rule_id");
CREATE INDEX "zing_doctor_db_prod"."idx_qcr_show" ON "zing_doctor_db_prod"."quality_count_rule" ("is_show_page", "sort_no");
...
-- BEGIN
--   EXECUTE IMMEDIATE
--     'INSERT INTO "zing_doctor_db_prod"."quality_count_rule" '
