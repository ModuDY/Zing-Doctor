...
BEGIN
    ...
    IF v_cnt = 0 THEN
        EXECUTE IMMEDIATE '
CREATE TABLE "zing_doctor_db_prod"."config_prone_item" (
    "id"           BIGINT DEFAULT "zing_doctor_db_prod"."SEQ_config_prone_item".NEXTVAL NOT NULL,
    ...
    CONSTRAINT "pk_config_prone_item" PRIMARY KEY ("id")
)';
    END IF;
...
COMMENT ON TABLE  "zing_doctor_db_prod"."config_prone_item" IS 'ARDS 俯卧位采集映射配置表（参数项 ← 数据源项目；表空或未命中回退字典内置关键字）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."config_type"  IS '数据源通道：observe_item 监护/呼吸机 / lis_item 检验/血气';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."config_key"   IS 'ARDS 参数编码（ArdsProneDict 的 key，如 hr / map / peep / pao2）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."config_value" IS '匹配值：item_code 列表 或 名称关键字列表（逗号分隔多值，任一命中即可）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."match_type"   IS '匹配方式：code 按 item_code 精确 / name 按项目名称包含（忽略大小写与空格）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."priority"     IS '优先级：数字小优先（同参数多渠道时表达「监护优先 / 检验兜底」）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."window_min"   IS '采集窗口覆盖（分钟），空则走字典默认（体征 ±15 / 检验 ±60）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."unit_scale"   IS '单位线性换算系数：值 × scale + offset（如 FiO₂ 0.4 → 40 时 scale=100）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."unit_offset"  IS '单位线性换算偏移量';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."item_name"    IS '项目名称（展示用）';
COMMENT ON COLUMN "zing_doctor_db_prod"."config_prone_item"."status"       IS '状态：1启用 0停用（停用项不参与取数，该参数回退内置关键字）';
...
BEGIN
    ...
    IF v_len IS NOT NULL AND v_len < 20 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE "zing_doctor_db_prod"."patient_doc_prone_record" MODIFY "admit_date" VARCHAR(20)';
    END IF;
    ...
    IF v_len IS NOT NULL AND v_len < 20 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE "zing_doctor_db_prod"."patient_doc_prone_record" MODIFY "record_date" VARCHAR(20)';
    END IF;
END;
/
...
--     FROM "zing_doctor_db_prod"."config_prone_item" ORDER BY "config_key", "priority";
--   SELECT "page_code", "page_name", "frontend_path", "status"
--     FROM "zing_doctor_db_prod"."sys_page_config" WHERE "page_code" = 'ards-prone-config';
