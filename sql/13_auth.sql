...
BEGIN
    SELECT COUNT(*) INTO v_cnt FROM ALL_SEQUENCES
     WHERE UPPER(SEQUENCE_OWNER) = 'ZING_DOCTOR_DB_PROD'
       AND UPPER(SEQUENCE_NAME)  = 'SEQ_SYS_USER';
    IF v_cnt = 0 THEN
        EXECUTE IMMEDIATE 'CREATE SEQUENCE "zing_doctor_db_prod"."SEQ_sys_user" START WITH 1 INCREMENT BY 1';
    END IF;
    ...
    IF v_cnt = 0 THEN
        EXECUTE IMMEDIATE 'CREATE TABLE "zing_doctor_db_prod"."sys_user" (
        ...
        EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX "uk_sys_user_name" ON "zing_doctor_db_prod"."sys_user" ("username")';
    END IF;
END;
/

COMMENT ON TABLE  "zing_doctor_db_prod"."sys_user" IS '直连登录账号表（用户名密码登录，区别于外链免登录）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_user"."username"        IS '登录账号（唯一）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_user"."real_name"       IS '姓名（页面右上角展示）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_user"."password_hash"   IS '口令散列：pbkdf2$迭代次数$盐(Base64)$摘要(Base64)';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_user"."status"          IS '状态：1启用 0停用';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_user"."last_login_time" IS '最近一次登录成功时间';
