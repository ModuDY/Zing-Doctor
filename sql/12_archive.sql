...
-- 1) 系统参数表
-- ---------------------------------------------------------------------
CREATE SEQUENCE "zing_doctor_db_prod"."SEQ_sys_param" START WITH 1 INCREMENT BY 1;
CREATE TABLE "zing_doctor_db_prod"."sys_param" (
    "id"           BIGINT DEFAULT "zing_doctor_db_prod"."SEQ_sys_param".NEXTVAL NOT NULL,
...
COMMENT ON TABLE  "zing_doctor_db_prod"."sys_param" IS '系统参数表（参数设置页面维护，如文书归档接口地址、归档目录）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_param"."param_key"   IS '参数键（唯一，如 ARCHIVE_API_URL / ARCHIVE_DIR）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_param"."param_name"  IS '参数名称（页面展示）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_param"."param_value" IS '参数值';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_param"."param_group" IS '参数分组（archive=文书归档）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_param"."sort_no"     IS '排序号';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_param"."status"      IS '状态：1启用 0停用';

-- ---------------------------------------------------------------------
-- 2) 归档推送流水表：记录每次归档/撤销的时间、file_path 与对方响应，便于追溯
-- ---------------------------------------------------------------------
CREATE SEQUENCE "zing_doctor_db_prod"."SEQ_sys_archive_log" START WITH 1 INCREMENT BY 1;
CREATE TABLE "zing_doctor_db_prod"."sys_archive_log" (
    "id"             BIGINT DEFAULT "zing_doctor_db_prod"."SEQ_sys_archive_log".NEXTVAL NOT NULL,
    "biz"            VARCHAR(32),
    "record_id"      BIGINT,
...
COMMENT ON TABLE  "zing_doctor_db_prod"."sys_archive_log" IS '文书归档推送流水';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."biz"          IS '业务：SOFA / APACHE2';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."record_id"    IS '评分记录 ID';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."doc_code"     IS '文书编码：sofa / apache2';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."score_date"   IS '评分日期（yyyy-MM-dd）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."file_path"    IS '按归档目录规则拼出的文件路径';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."op_type"      IS '操作：push=归档推送 unmark=撤销标记';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."success"      IS '是否成功：1成功 0失败';
COMMENT ON COLUMN "zing_doctor_db_prod"."sys_archive_log"."resp_message" IS '失败原因 / 对方返回报文摘要';
...
COMMENT ON COLUMN "zing_doctor_db_prod"."patient_doc_sofa_score_record"."archive_status" IS '归档状态：0待归档 1已归档';
COMMENT ON COLUMN "zing_doctor_db_prod"."patient_doc_sofa_score_record"."archive_time"   IS '最近一次归档成功时间';
COMMENT ON COLUMN "zing_doctor_db_prod"."patient_doc_sofa_score_record"."file_path"      IS '文书归档路径（按参数设置页的归档目录规则生成）';
...
COMMENT ON COLUMN "zing_doctor_db_prod"."patient_doc_apache2_score_record"."archive_status" IS '归档状态：0待归档 1已归档';
COMMENT ON COLUMN "zing_doctor_db_prod"."patient_doc_apache2_score_record"."archive_time"   IS '最近一次归档成功时间';
COMMENT ON COLUMN "zing_doctor_db_prod"."patient_doc_apache2_score_record"."file_path"      IS '文书归档路径（按参数设置页的归档目录规则生成）';
