-- =====================================================================
-- zing-doctor 医生系统 - SOFA 评分增量脚本（达梦 DM8）
--
-- 内容：
...
CREATE TABLE "zing_doctor_db_prod"."sofa_score_record" (
    ...
    CONSTRAINT "pk_sofa_score_record" PRIMARY KEY ("id")
);
COMMENT ON TABLE  "zing_doctor_db_prod"."sofa_score_record" IS 'SOFA 评分记录表';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."patient_id"          IS '患者ID（patient_info.id）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."in_hospital_no"      IS '住院号';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."patient_name"        IS '患者姓名';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."depart_code"         IS '科室编码';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."score_time"          IS '评分时间';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."score_type"          IS '来源：auto 系统自动 / daily 每日 / custom 自定义';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."resp_score"          IS '呼吸评分 0-4（PaO2/FiO2）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."coag_score"          IS '凝血评分 0-4（血小板）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."liver_score"         IS '肝评分 0-4（总胆红素）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."cardio_score"        IS '循环评分 0-4（MAP/血管活性药）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."neuro_score"         IS '神经评分 0-4（GCS）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."renal_score"         IS '肾评分 0-4（肌酐/24h尿量）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."total_score"         IS 'SOFA 总分 0-24（6 项之和）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."resp_data"           IS '呼吸项原始值 JSON（含取值时间）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."coag_data"           IS '凝血项原始值 JSON';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."liver_data"          IS '肝项原始值 JSON';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."cardio_data"         IS '循环项原始值 JSON';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."neuro_data"          IS '神经项原始值 JSON';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."renal_data"          IS '肾项原始值 JSON';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."vasopressor_json"    IS '血管活性药明细 JSON（药名/归一剂量 µg-kg-min/泵速/浓度/是否降级）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."urine_ml"            IS '取数窗口内尿量合计（mL）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."gcs_total"           IS 'GCS 总分（3-15），未评全为 NULL';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."gcs_detail"          IS 'GCS 明细 E/V/M';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."respiratory_support" IS '是否有呼吸支持：1是 0否';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."weight_used"         IS '本次剂量换算所用体重（kg）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."weight_source"       IS '体重来源：actual 实际 / ibw 身高估算 / default_age_sex 年龄性别默认 / fallback70 兜底';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."delta_sofa"          IS '较上一次评分的总分变化（用于恶化预警）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."data_start_time"     IS '取数开始时间';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."data_end_time"       IS '取数结束时间';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."remark"              IS '备注（含体重来源、剂量降级等提示）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_score_record"."status"              IS '状态：1正常 0已删除';
CREATE INDEX "zing_doctor_db_prod"."idx_sofa_patient" ON "zing_doctor_db_prod"."sofa_score_record" ("in_hospital_no");
CREATE INDEX "zing_doctor_db_prod"."idx_sofa_depart"  ON "zing_doctor_db_prod"."sofa_score_record" ("depart_code");
CREATE INDEX "zing_doctor_db_prod"."idx_sofa_time"    ON "zing_doctor_db_prod"."sofa_score_record" ("score_time");
...
CREATE TABLE "zing_doctor_db_prod"."sofa_config" (
    ...
    "create_time"  TIMESTAMP    DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "update_time"  TIMESTAMP    DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT "pk_sofa_config" PRIMARY KEY ("id")
);
COMMENT ON TABLE  "zing_doctor_db_prod"."sofa_config" IS 'SOFA 配置表';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_config"."config_type"  IS '配置类型：lis_item 检验项 / observe_item 监护项 / io_item 出入量项 / vasopressor 血管活性药 / conversion 换算系数 / default_weight 默认体重';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_config"."config_key"   IS '配置键';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_config"."config_value" IS '配置值';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_config"."item_name"    IS '项目名称（展示用）';
COMMENT ON COLUMN "zing_doctor_db_prod"."sofa_config"."remark"       IS '备注';
CREATE INDEX "zing_doctor_db_prod"."idx_sofa_config_type" ON "zing_doctor_db_prod"."sofa_config" ("config_type", "status");
...
INSERT INTO "zing_doctor_db_prod"."sofa_config"
...
SELECT 'lis_item', 'platelet', '200050', '血小板', '已现场确认；单位 10^9/L 与 SOFA ×10^3/µL 数值等同', 1 FROM DUAL
 WHERE NOT EXISTS (SELECT 1 FROM "zing_doctor_db_prod"."sofa_config"
