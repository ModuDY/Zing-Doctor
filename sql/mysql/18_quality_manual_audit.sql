...
CALL zing_add_column('quality_metric_result', 'value_source', 'VARCHAR(16) DEFAULT ''AUTO'' COMMENT ''值归属：AUTO=引擎计算 / MANUAL=人工录入（重算时受保护，不被覆盖）''');
CALL zing_add_column('quality_metric_result', 'operator', 'VARCHAR(64) COMMENT ''人工录入的操作人（审计留痕；引擎写入的行此列为空）''');
CALL zing_add_column('quality_metric_result', 'manual_note', 'VARCHAR(500) COMMENT ''人工录入备注（说明取数依据，便于事后复核）''');
