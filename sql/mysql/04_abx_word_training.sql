...
-- 验证：训练后词库统计
SELECT 'broad_spectrum' AS word_type, COUNT(*) AS cnt FROM `config_abx_word` WHERE `word_type` = 'broad_spectrum' AND `status` = 1
UNION ALL
SELECT 'non_antibiotic', COUNT(*) FROM `config_abx_word` WHERE `word_type` = 'non_antibiotic' AND `status` = 1;
