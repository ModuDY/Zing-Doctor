...
-- 1) SOFA 评分记录表
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `patient_doc_sofa_score_record` (
...
CALL zing_add_index('patient_doc_sofa_score_record', 'idx_sofa_patient', 0, '`in_hospital_no`');
CALL zing_add_index('patient_doc_sofa_score_record', 'idx_sofa_depart', 0, '`depart_code`');
CALL zing_add_index('patient_doc_sofa_score_record', 'idx_sofa_time', 0, '`score_time`');

-- ---------------------------------------------------------------------
-- 2) SOFA 配置表（表空时才写入默认种子，不覆盖院内调整）
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `config_sofa` (
  `id` BIGINT AUTO_INCREMENT NOT NULL,
...
CALL zing_add_index('config_sofa', 'idx_config_sofa_type', 0, '`config_type`, `status`');
...
SELECT 'lis_item', 'platelet', '200050', '血小板', '已现场确认；单位 10^9/L 与 SOFA ×10^3/µL 数值等同', 1 FROM DUAL
 WHERE NOT EXISTS (SELECT 1 FROM `config_sofa`
