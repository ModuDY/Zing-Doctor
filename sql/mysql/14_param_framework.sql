...
CALL zing_add_column('sys_param', 'param_type', 'VARCHAR(20) DEFAULT ''text'' COMMENT ''参数类型：text/textarea/number/switch/select，页面据此渲染控件''');
CALL zing_add_column('sys_param', 'options', 'VARCHAR(1000) COMMENT ''select 选项 JSON 数组：[{`label`:`显示名`,`value`:`存储值`}]''');
CALL zing_add_column('sys_param', 'default_value', 'VARCHAR(1000) COMMENT ''默认值（参数值为空时取用）''');
CALL zing_add_column('sys_param', 'required', 'TINYINT DEFAULT 0 COMMENT ''是否必填：1 必填 / 0 选填''');
CALL zing_add_column('sys_param', 'regex', 'VARCHAR(200) COMMENT ''校验正则（可选，保存时校验参数值）''');
CREATE TABLE IF NOT EXISTS `sys_param_group` (
  `id` BIGINT       AUTO_INCREMENT NOT NULL,
  `group_code` VARCHAR(64)  NOT NULL COMMENT '分组编码（唯一，与 sys_param.param_group 对应）',
...
CALL zing_add_index('sys_param_group', 'uk_sys_param_group_code', 1, '`group_code`');
...
SELECT 'archive', '文书归档', 10, 'Folder',
  ...
  FROM DUAL
 WHERE NOT EXISTS (
    SELECT 1 FROM `sys_param_group` WHERE `group_code` = 'archive');
...
SELECT 'score', '评分配置', 30, 'EditPen',
  ...
  FROM DUAL
 WHERE NOT EXISTS (
    SELECT 1 FROM `sys_param_group` WHERE `group_code` = 'score');
...
SELECT 'system', '系统设置', 90, 'Setting',
  ...
  FROM DUAL
 WHERE NOT EXISTS (
    SELECT 1 FROM `sys_param_group` WHERE `group_code` = 'system');
...
SELECT 'AUTO_REGISTER_ENABLED', '外链工号自动注册', '0', 'external', 10, 1, 'switch',
  ...
  FROM DUAL
 WHERE NOT EXISTS (
    SELECT 1 FROM `sys_param` WHERE `param_key` = 'AUTO_REGISTER_ENABLED');
...
SELECT 'AUTO_REGISTER_PASSWORD_RULE', '自动注册默认口令规则', 'WORK_NO', 'external', 20, 1, 'select',
  ...
  FROM DUAL
 WHERE NOT EXISTS (
    SELECT 1 FROM `sys_param` WHERE `param_key` = 'AUTO_REGISTER_PASSWORD_RULE');
...
SELECT 'AUTO_REGISTER_FIXED_PASSWORD', '自动注册固定初始口令', '', 'external', 30, 1, 'text',
  ...
  FROM DUAL
 WHERE NOT EXISTS (
    SELECT 1 FROM `sys_param` WHERE `param_key` = 'AUTO_REGISTER_FIXED_PASSWORD');

-- ---------------------------------------------------------------------
-- 执行后自检（可选）
--   SELECT `group_code`, `group_name`, `sort_no` FROM `sys_param_group` ORDER BY `sort_no`;
--   SELECT `param_key`, `param_value`, `param_type` FROM `sys_param` ORDER BY `param_group`, `sort_no`;
-- ---------------------------------------------------------------------
