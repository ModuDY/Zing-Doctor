...
--
-- 说明：
--   医生系统统一使用 SYSDBA 连接达梦（与 ICU 系统访问方式一致），
--   通过显式模式前缀访问各模式。
--   本脚本创建医生系统自有模式 zing_doctor_db_prod（属主 SYSDBA），
--   医生系统自有表（zing_page_config 等）建于此模式下，不使用 SYSDBA 默认模式。
--   ICU 系统库模式（zing_icu_db_prod）由 ICU 只读数据源访问，本脚本不触碰。
-- =====================================================================

-- 创建医生系统自有模式（幂等：DbInit 会先查 SYS.SYSOBJECTS，模式已存在则跳过创建；
