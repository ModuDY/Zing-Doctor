...
public class ExternalLinkInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (OperatorContext.isPlaceholder(operator)) {
            // 外链没带身份时的兜底，与直连登录通道的「退回账号」对齐。
            // 此前这里直接把空值写进 attribute，业务层遂一律记成 unknown ——
            // 现场表现是评分记录、变更历史里全是 unknown，看着像系统坏了，
            // 实际是 ICU 侧外链模板没配 realname，且服务端没有任何可配置的替代值。
            // 不做成「猜一个人名」：拿不到就是拿不到，这里只提供一个可辨识的来源标签。
            String fallback = properties.getDefaultOperator();
            operator = OperatorContext.isPlaceholder(fallback) ? null : fallback.trim();
        }
     ...
     *
     * <p>占位值（空、{@code ${realname}} 这类没被替换的模板变量、unknown 及其拼错写法）
    ...
    private String operatorFromExternal(HttpServletRequest request, ExternalLinkContext ctx) {
        ...
        for (String key : keys) {
            String value = ctx.param(key);
            if (!OperatorContext.isPlaceholder(value)) {
                return value.trim();
            }
        }
        return null;
