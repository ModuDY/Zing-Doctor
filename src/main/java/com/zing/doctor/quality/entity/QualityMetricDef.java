...
public class QualityMetricDef {
    ...
    /**
     * 分子 / 分母取自另一条指标的结果（率类跨事实层用），存被引用指标的编码。
     *
     * <p>例：「ICU患者预计病死率」的分子取 {@code quality_15}、分母取 {@code quality_403}。
     * 配了引用则不编译本指标的事实层 SQL，由引擎在批次里按科室取被引用指标的行相除。
     */
    ...
    /**
     * 患者明细的补充展示字段，JSON 数组字符串，如
     * {@code [{"key":"gender","label":"性别","width":70}]}；空表示只用默认列。
     *
     * <p>存 JSON 而不是拆列：字段个数与含义随指标变，拆列会立刻遇到「最多几个」的天花板。
     */
    private String patientFields;
